...
generate_prompt() {
  ...
  # regenera se qualquer formato estiver faltando
  ...
  if command -v espeak-ng >/dev/null 2>&1 && command -v sox >/dev/null 2>&1; then
    ...
    # WAV PCM 8kHz mono 16-bit
    sox "$tmp_file" -r 8000 -c 1 -b 16 -e signed-integer "$output_file"
    # ulaw raw 8kHz mono — formato nativo do Asterisk, funciona sem format_wav
...
generate_prompt \
  ...
  "Olá. Bem-vindo ao atendimento EdaCall. Digite um para vendas, dois para suporte técnico, ou três para financeiro."

generate_prompt \
  /var/lib/asterisk/sounds/custom/edacall-opcao-invalida-ptbr.wav \
  "Opção inválida ou nenhuma opção digitada. Encerrando atendimento."
