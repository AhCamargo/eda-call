...
export default function AgentView() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/extensions/${selectedExtId}/webrtc-credentials`)
  ...
  const isInCall =
    sip.callState === "in_call" || selectedExt?.status === "in_call";
  const isRinging =
    sip.callState === "ringing_in" || selectedExt?.status === "ringing";
  const isCallingOut = sip.callState === "ringing_out";
  const isPaused = selectedExt?.status === "paused";

  const statusCfg = selectedExt
    ? STATUS_CONFIG[selectedExt.status as keyof typeof STATUS_CONFIG] ||
  ...
  const myHistory = useMemo(() => {
    ...
    return reportCallsByExtension
      .filter(
        (c) =>
          c.extensionId === selectedExt.id ||
          c.Extension?.id === selectedExt.id,
      )
      .slice(0, 8);
  ...
  const handlePause = () => {
    if (selectedExtId) pauseExtension(selectedExtId, pauseReason);
  };
  const handleResume = () => {
    if (selectedExtId) resumeExtension(selectedExtId);
  };
  ...
  const resultIcon = (result: string) => {
    ...
    if (result === "nao_atendida")
      return <PhoneMissed size={13} className="text-red-400" />;
        ...
        <CardContent className="pt-5">
          <div className="flex items-center gap-3">
                ...
                {extensions.map((ext) => (
                  ...
                  >
                    <span className="flex items-center gap-2">
                      <span
                        className={`w-2 h-2 rounded-full ${STATUS_CONFIG[ext.status as keyof typeof STATUS_CONFIG]?.color || "bg-zinc-500"}`}
                      />
                      {ext.number} — {ext.name}
                    </span>
                  </SelectItem>
            ...
            {selectedExt && statusCfg && (
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full mr-1.5 ${statusCfg.color}`}
                />
                {statusCfg.label}
              </Badge>
            )}
            ...
            {selectedExtId && (
              <div
                className={`flex items-center gap-1.5 text-xs ${sipCfg.color} flex-shrink-0`}
              >
                <SipIcon
                  size={13}
                  className={
                    sip.sipStatus === "connecting" ? "animate-spin" : ""
                  }
                />
              ...
              {totalToday > 0 && (
                <div>
                  <div className="flex justify-between text-xs mb-1 text-zinc-400">
                    ...
                    <span>
                      {Math.round((answeredToday / totalToday) * 100)}%
                    </span>
                  </div>
                  <Progress
                    value={(answeredToday / totalToday) * 100}
                    className="h-1.5"
                  />
                </div>
              )}
        ...
        <CardContent>
          {myHistory.length > 0 ? (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <Badge
                      variant="outline"
                      className={`text-xs ${
                        call.result === "atendida"
                          ? "text-green-400 border-green-500/30"
                          : call.result === "nao_atendida"
            ...
            <p className="text-zinc-500 text-sm text-center py-6">
              {selectedExt
                ? "Nenhuma ligação registrada para este ramal"
                : "Selecione um ramal para ver o histórico"}
            </p>
          )}
        </CardContent>
      </Card>
