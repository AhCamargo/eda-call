...
function Dashboard() {
  ...
  const ongoingCallAgents = useMemo(
    () =>
      extensions
        .filter((ext) => ext.status === "in_call" || ext.status === "ringing")
        .slice(0, 6),
    [extensions],
  );
  ...
  const breakAgents = useMemo(
    () =>
      extensions
        .filter((ext) => ext.status === "paused" || ext.status === "offline")
        .slice(0, 6),
    [extensions],
  );
  ...
  const totalRelevantCalls =
    (reports.quemAtendeu || 0) + (reports.numeroNaoExiste || 0);
  ...
  const routedCalls = Math.max(
    0,
    callsReceived - answeredCalls - abandonedCalls,
  );
  const serviceLevel = callsReceived
    ? Math.round((answeredCalls / callsReceived) * 100)
    : 0;
  const callsInQueue = ringingCalls;
  const oldestInQueueSeconds = Math.max(0, callsInQueue * 14);
  const oldestMinutes = String(Math.floor(oldestInQueueSeconds / 60)).padStart(
    2,
    "0",
  );
  const oldestSeconds = String(oldestInQueueSeconds % 60).padStart(2, "0");
  const abandonRate = callsReceived
    ? Math.round((abandonedCalls / callsReceived) * 100)
    : 0;
  ...
  const agentStateItems = [
    {
      label: t("dashboard.notReady"),
      value: statusCounts.paused || 0,
      color: "error.main",
    },
    {
      label: t("dashboard.talking"),
      value: activeCalls,
      color: "warning.main",
    },
    {
                    ...
                    <Grid container spacing={2} sx={{ mb: 2 }}>
                      <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                        ...
                        <Typography sx={{ color: "success.light" }}>
                          {t("dashboard.answered")}: {answeredCalls}
                        </Typography>
                      </Grid>
                      ...
                      <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                        ...
                        <Typography sx={{ color: "warning.light" }}>
                          {t("dashboard.oldest")}: {oldestMinutes}:
                          {oldestSeconds}
                        </Typography>
                      </Grid>
                    </Grid>
                    ...
                    <Grid container spacing={2}>
                      <Grid size={{ xs: 12, md: 3 }}>
                        <Typography variant="subtitle2" sx={{ mb: 1 }}>
                          {t("dashboard.incomingDistribution")}
                        </Typography>
                        <Stack spacing={1}>
                          {[
                            {
                            ...
                            {
                              label: t("dashboard.abandoned"),
                              value: abandonedCalls,
                              color: "error.main",
                            },
                            {
                              label: t("dashboard.routed"),
                              value: routedCalls,
                              color: "info.main",
                            },
                          ].map((item) => (
                          ...
                          {topAgents.length ? (
                            topAgents.map((agent) => (
                              ...
                              >
                                <Box
                                  sx={{
                                    width: `${Math.min(100, agent.score)}%`,
                                    maxWidth: 130,
                                    height: 10,
                      ...
                      {activitySeries.map((value, index) => (
                        <LinearProgress
                          ...
                          sx={{
                            ...
                            bgcolor: alpha(theme.palette.common.white, 0.2),
                            "& .MuiLinearProgress-bar": {
                              bgcolor: alpha(theme.palette.success.light, 0.9),
                            },
                          }}
                        />
                      ))}
                    ...
                    <Grid container spacing={1.2}>
                      {ongoingCallAgents.length ? (
                        ongoingCallAgents.map((ext) => (
                          <Grid key={ext.id} size={{ xs: 12, sm: 6, md: 4 }}>
                            <Card variant="outlined">
                        ...
                        allExtensions.map((ext) => (
                          ...
                          >
                            ...
                            <Chip
                              size="small"
                              label={ext.status}
                              color={getStatusColor(ext.status)}
                            />
                          </Stack>
                        ))
                      ) : (
          ...
          <Stack spacing={2}>
            ...
            <Card>
              <CardContent>
                ...
                <Divider sx={{ my: 1 }} />
                <Typography>
                  {t("dashboard.whoAnswered")}: {reports.quemAtendeu}
                </Typography>
                <Typography>
                  {t("dashboard.numberNotExists")}: {reports.numeroNaoExiste}
                </Typography>
